namespace LibraryPhysicalUnits8may2024.Test
{
    public class UnitTestVelocity9may2024
    {
        ...
        public void Test12()
        {
            ...
            Console.WriteLine("a)");
            var v = new VelocityInKilometerPerHour6apr2024(95, 1);
            var reactietijd = new TimeInSeconds8may2024(1, 0.1);
            var a1 = new AccelerationInMeterPerSecondSquared17may2024(-5, 0.1);
            var s1 = v * reactietijd;
            var t2_a = v / a1;
            var s2_a = -a1 * t2_a.Square() / 2;
            var s_a = s1 + s2_a;
            Console.WriteLine("s = " + s_a); // 96 meter
            Assert.Equal(expected: 96, actual: s_a.GetInMeter(), tolerance: 1);
            ...
            var s_b = s1 + s2_b;
            Console.WriteLine("s = " + s_b); // 76 meter
            Assert.Equal(expected: 76, actual: s_b.GetInMeter(), tolerance: 1);
        }
    }
}